...
const conceptsTable = [
  ...
  { concept: 'String Variable',        what: 'Temporary value storage during pipeline run',       when: 'When you need to pass values between activities' },
...
const newItems = [
  ...
  { item: 'Variable',  name: 'current_file_name (String)',desc: 'Temporary storage during loop' },
  ...
  return (
    <LearnLayout
      ...
      breadcrumbs={[
        ...
        { label: 'Project 02 — ForEach Loop', href: '/learn/projects/azure-project-02' },
      ]}
    ...
    >
      ...
      <div className="my-4 flex gap-4">
        {[
          ...
          <div key={m.mode} className="flex-1 p-3 rounded-xl text-xs" style={{ background: 'var(--surface)', border: `1px solid ${m.color}30` }}>
      ...
      <h3>Parameters vs Variables — What is the Difference?</h3>
      <div className="my-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
        {[
          { label: 'PARAMETER', color: '#0078d4', points: ['Set FROM OUTSIDE the pipeline', 'Set before the pipeline starts', 'Cannot be changed during run', 'Example: run_date = "2024-01-15" (set when triggering)'] },
          { label: 'VARIABLE',  color: '#7b61ff', points: ['Set INSIDE the pipeline', 'Can change during pipeline run', 'Can be read and changed any time', 'Example: current_file = "" (pipeline sets this in each loop)'] },
        ].map(item => (
          <div key={item.label} className="p-4 rounded-xl" style={{ background: 'var(--surface)', border: `1px solid ${item.color}30` }}>
            <div className="text-xs font-mono font-bold mb-3" style={{ color: item.color }}>{item.label}</div>
      ...
      <p>
        In Project 01, our dataset had a hardcoded file name: <code>daily_sales.csv</code> — fixed, never changes. In Project 02, the file name changes on every loop iteration. This is where dynamic expressions come in.
      </p>
      <p>
      ...
      <div className="my-4 p-4 rounded-xl font-mono text-xs space-y-2" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        ...
        {[
          ...
          ['@pipeline().parameters.run_date',           'Value of a pipeline parameter'],
          ["@variables('current_file')",                'Value of a variable'],
          ['@utcNow()',                                  'Current date and time'],
          ["@formatDateTime(utcNow(),'yyyy-MM-dd')",    "Today's date formatted"],
        ].map(([expr, desc]) => (
          <div key={expr} className="flex gap-4">
            <span className="flex-shrink-0" style={{ color: '#00e676', width: 280 }}>{expr}</span>
      ...
      {/* Pipeline diagram */}
      <div className="my-6 p-5 rounded-xl font-mono text-xs" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
      ...
      <p>Each file represents one store's daily sales. Open Notepad (Windows) or TextEdit (Mac) and create each file below. Save them all in a folder on your Desktop called <code>freshmart_store_files</code>.</p>
      ...
      {Object.entries(csvFiles).map(([filename, content]) => (
        <div key={filename} className="my-4 rounded-xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
          ...
          <pre className="p-4 text-xs overflow-x-auto" style={{ background: 'var(--bg2)', color: 'var(--text2)', lineHeight: 1.6 }}>{content}</pre>
        </div>
      ))}

      <p>Create files 3–10 with the same structure, using the store IDs ST003 through ST010 and their respective city product data as shown in the document above. Save all 10 as <code>store_ST00X_sales.csv</code>.</p>
      ...
      <p>Click into the <strong>store_sales</strong> directory → click <strong>"Upload"</strong> → hold Ctrl and select all 10 CSV files at once → click <strong>"Upload"</strong>.</p>
      ...
      <p className="mt-4">
        In Project 01, our datasets had a hardcoded file path: <code>File: daily_sales.csv</code>. That worked for one file. Now the file name needs to change on each loop iteration. We do this by adding <strong>parameters to the dataset</strong> — a placeholder instead of a fixed value.
      </p>
      ...
      <p>Now click the <strong>"Connection"</strong> tab → click inside the <strong>"File"</strong> field → click <strong>"Add dynamic content"</strong> (blue link below the field).</p>
      ...
      <div className="my-3 p-3 rounded-xl font-mono text-sm" style={{ background: 'var(--bg2)', border: '1px solid var(--border)', color: '#00e676' }}>
        @dataset().file_name
      </div>
      ...
      <p>Click <strong>"+"</strong> next to Datasets → <strong>"New dataset"</strong> → <strong>"Azure Data Lake Storage Gen2"</strong> → <strong>"Continue"</strong> → <strong>"DelimitedText"</strong> → <strong>"Continue"</strong>.</p>
      <div className="my-4 p-4 rounded-xl font-mono text-xs space-y-2" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
      ...
      <p>Click <strong>Connection</strong> tab → <strong>File</strong> field → <strong>"Add dynamic content"</strong> → click <strong>file_name</strong> → expression: <code>@dataset().file_name</code> → <strong>"OK"</strong>.</p>
      <div className="my-4 p-4 rounded-xl font-mono text-xs space-y-2" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
      ...
      <p>Click on the <strong>empty canvas background</strong> (not on any activity) → at the bottom → click the <strong>"Parameters"</strong> tab.</p>
      ...
      <Callout type="warning" label="⚠️ Array Format Must Be Exact JSON">
        The default value must be a valid JSON array: strings in <strong>double quotes</strong>, separated by commas, wrapped in square brackets. Copy the value above exactly — no trailing commas, no single quotes.
      </Callout>
      ...
      <h2>Step 7 — Add Pipeline Variable</h2>
      <p>Click on the empty canvas again → click the <strong>"Variables"</strong> tab at the bottom (next to Parameters) → <strong>"+ New"</strong>:</p>
      <div className="my-4 p-4 rounded-xl font-mono text-xs space-y-2" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <div className="flex gap-4"><span style={{ color: 'var(--muted)', width: 100, flexShrink: 0 }}>Name</span><span style={{ color: '#00e676' }}>current_file_name</span></div>
        <div className="flex gap-4"><span style={{ color: 'var(--muted)', width: 100, flexShrink: 0 }}>Type</span><span style={{ color: '#00e676' }}>String</span></div>
        <div className="flex gap-4"><span style={{ color: 'var(--muted)', width: 100, flexShrink: 0 }}>Default</span><span style={{ color: 'var(--muted)' }}>leave empty</span></div>
      </div>
      <Screenshot caption="Variables tab — current_file_name variable of type String added" />

      <h2>Step 8 — Add ForEach Activity</h2>
      <p>In the left activities panel → expand <strong>"Iteration & conditionals"</strong> → drag <strong>"ForEach"</strong> onto the canvas.</p>
      ...
      <p>For <strong>Items</strong> field → click <strong>"Add dynamic content"</strong> → under <strong>"Parameters"</strong> → click <strong>store_files</strong>. Expression becomes <code>@pipeline().parameters.store_files</code>.</p>
      ...
      <h2>Step 9 — Add Copy Activity INSIDE the ForEach</h2>
      <Callout type="warning" label="⚠️ Most Common Mistake in This Step">
        Do NOT drag a Copy activity from the left panel onto the main canvas. You must click the <strong>"+"</strong> that is <strong>inside</strong> the ForEach box. This is the mistake beginners make most often.
      </Callout>
      ...
      <h2>Step 10 — Configure Source with Dynamic Expression</h2>
      ...
      <h3>Source Tab</h3>
      <p>Select <code>ds_src_blob_store_sales</code>. A <strong>Dataset properties</strong> section appears. Click inside the <strong>file_name</strong> value field → <strong>"Add dynamic content"</strong>.</p>
      ...
      <div className="my-4 p-4 rounded-xl text-xs" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        ...
        <p style={{ color: 'var(--text2)' }}><code>@item()</code> only works inside a ForEach loop. It returns the current item being processed.</p>
      ...
      <h2>Step 11 — Configure Sink with Dynamic Expression</h2>
      <p>Click <strong>Sink</strong> tab → select <code>ds_sink_adls_store_sales</code> → in the <strong>file_name</strong> Dataset property → <strong>"Add dynamic content"</strong> → click <strong>"Item"</strong> → expression: <code>@item()</code> → <strong>"OK"</strong>.</p>
      <Screenshot caption="Sink tab — ds_sink_adls_store_sales selected, file_name Dataset property showing @item()" />
      <p>Both source and sink now use <code>@item()</code> — the same file name is used for reading and writing:</p>
      <div className="my-4 p-4 rounded-xl font-mono text-xs space-y-2" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <div className="flex gap-4"><span style={{ color: 'var(--muted)', width: 60, flexShrink: 0 }}>Source</span><span style={{ color: 'var(--text2)' }}>reads from</span><span className="ml-2" style={{ color: '#ff9900' }}>landing/store_sales/store_ST001_sales.csv</span></div>
        <div className="flex gap-4"><span style={{ color: 'var(--muted)', width: 60, flexShrink: 0 }}>Sink</span><span style={{ color: 'var(--text2)' }}>writes to</span><span className="ml-2" style={{ color: '#00e676' }}>raw/sales/store_ST001_sales.csv</span></div>
        <p className="text-xs pt-1" style={{ color: 'var(--muted)' }}>Same file name — different container and folder. Clean.</p>
      </div>

      <h2>Step 12 — Validate and Return to Main Canvas</h2>
      <p>Click the <strong>back arrow</strong> at the top left of the inner canvas to return to the main pipeline canvas.</p>
      ...
      <h2>Step 13 — Debug</h2>
      <p>Click <strong>"Debug"</strong>. A dialog appears asking for parameter values — the default array should be pre-filled. If not, paste it:</p>
      <div className="my-3 p-3 rounded-xl font-mono text-xs" style={{ background: 'var(--bg2)', border: '1px solid var(--border)', color: '#7b61ff', wordBreak: 'break-all' }}>
        {defaultArray}
      </div>
      <Screenshot caption="Debug parameter dialog — store_files parameter with the JSON array value pre-filled" />
      <p>Click <strong>"OK"</strong>. Watch the ForEach run. Click the <strong>👓 glasses icon</strong> next to the ForEach in the Output tab to see individual iterations.</p>
      ...
      <h2>Step 14 — Verify All 10 Files in ADLS</h2>
      ...
      <div className="my-8 p-5 rounded-xl" style={{ background: 'var(--surface)', border: '1px solid var(--accent2)' }}>
        <div className="text-xs font-mono uppercase tracking-widest mb-3" style={{ color: 'var(--accent2)' }}>💡 Bonus — Test With Only 3 Files</div>
        <p className="text-sm mb-3" style={{ color: 'var(--text2)' }}>
          Click <strong>Debug</strong> → change the <code>store_files</code> value to just 3 files. The pipeline runs only 3 iterations. This is how professionals test with a subset before running the full load.
        </p>
        <div className="p-3 rounded-lg font-mono text-xs" style={{ background: 'var(--bg2)', color: '#7b61ff', wordBreak: 'break-all' }}>
      ...
      <div className="space-y-3 my-6">
        {[
          ...
          { mistake: 'Using @variables() instead of @item() inside ForEach',   fix: 'Use @item() to get the current loop value inside ForEach' },
        ].map((item, i) => (
      ...
      <div className="my-8 p-6 rounded-xl" style={{ background: 'var(--surface)', border: '2px solid var(--accent2)' }}>
        <div className="text-xs font-mono uppercase tracking-widest mb-3" style={{ color: 'var(--accent2)' }}>What is coming in Project 03</div>
        <p className="text-sm mb-3" style={{ color: 'var(--text)' }}>
          Right now you pass the file list as a hardcoded default array parameter. Every time the list changes, you edit the pipeline. What if the file name includes today's date?
        </p>
        ...
        <p className="text-sm" style={{ color: 'var(--text2)' }}>
          In <strong>Project 03</strong> you will learn <strong>Parameterized Pipelines</strong> — pass <code>run_date</code> at trigger time and ADF builds the correct file name automatically. The pipeline runs every night and picks up the right files with zero manual input.
        </p>
      </div>
      ...
      <KeyTakeaways items={[
        ...
        'Parameters are set from outside the pipeline before it runs — Variables are set and changed during the run',
