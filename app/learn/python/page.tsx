...
const phaseInfo = PYTHON_CURRICULUM.map(s => ({ id: s.id, title: s.title }))

const modules: Module[] = PYTHON_CURRICULUM.flatMap(s => s.modules.map(m => ({
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {[
          { icon: '🎓', label: 'Complete beginners — zero programming knowledge required' },
          { icon: '🔄', label: 'Developers switching into Python from another language' },
          { icon: '💼', label: 'Anyone preparing for Python interviews' },
          { icon: '🚀', label: 'Students who want production depth, not just syntax' },
        ].map((item) => (
      ...
      <div style={{
        ...
        <strong style={{ color: 'var(--accent)' }}>General-purpose Python, not tool-specific.</strong>{' '}
        This track takes you from your first script to production-grade Python — syntax, data
        structures, OOP, functional patterns, concurrency, testing, and packaging. Python for Data
        Engineering and Python for ML have their own dedicated lessons inside those tracks; this is
        the foundation underneath all of them.
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 28 }}>
        {filtered.map((mod, idx) => {
          ...
          const href   = isLive ? `/learn/python/${mod.slug}` : '#'
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href="/learn/python/what-is-python-setup" style={{
