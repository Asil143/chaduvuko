...
const STRATUM_NODES: StratumNode[] = [
  ...
  { stratum: 1, label: 'Primary Time Servers (Stratum 1)', examples: ['time.nist.gov', 'time.cloudflare.com', 'GPS-backed enterprise server'], accuracy: '±1–100 microseconds', description: 'Computers directly connected to a Stratum 0 reference clock. They speak NTP and serve time to Stratum 2 servers. These are the root of the NTP hierarchy. Running your own Stratum 1 requires a GPS receiver and careful tuning.', color: '#8b5cf6' },
  { stratum: 2, label: 'Secondary Time Servers (Stratum 2)', examples: ['Corporate NTP servers', 'ISP time servers', 'many pool.ntp.org members'], accuracy: '±1–10 milliseconds', description: 'Servers that synchronize from multiple Stratum 1 sources. Most organization NTP servers are Stratum 2. Public pool.ntp.org members vary by stratum; pool membership is not a guaranteed tier. For most purposes, Stratum 2 provides sufficient accuracy.', color: '#3b82f6' },
...
const NTP_FIELDS: NtpPacketField[] = [
  ...
  { field: 'Reference Timestamp', value: '2026-05-24T10:00:00Z', bits: '64 bits', description: 'When the system clock was last set or corrected. NTP timestamp format: 64-bit fixed-point with 32-bit seconds (since 1900-01-01) and 32-bit fractional seconds (2^32 units per second, about 233 ps per unit).' },
...
refclock PHC /dev/ptp0 poll 2 dpoll -2 offset 0`}</CodeBlock>
      ...
      <IQ level="PhD">
        ...
        NTS uses a two-phase approach. Phase 1 (NTS-KE): the client connects to the NTP server's NTS-KE port (TCP/4460) and performs a TLS 1.3 handshake. The server authenticates itself via its TLS certificate (verifiable against the CA store — no first-use trust problem), and the TLS handshake provides forward secrecy for key establishment. The TLS application-layer protocol negotiation (ALPN) extension identifies the NTS-KE protocol. Over the TLS session, the server sends the client a set of "cookies" — opaque blobs that encode symmetric key material encrypted with the NTS server's key, so only the server can decode them. Phase 2 (NTP): the client adds an NTS extension field to each NTP packet containing one cookie (key identifier + AEAD ciphertext) and a fresh-nonce authenticated MAC. The server decodes the cookie to recover the session key, verifies the MAC, then sends a response with a new cookie (to prevent replay). The client uses AEAD (AES-SIV or ChaCha20-Poly1305) to authenticate both request and response. Cookies support stateless operation and should be rotated/refreshed; the server maintains no per-client state between exchanges because the cookie carries what it needs.
      </IQ>
