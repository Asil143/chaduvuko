...
const RACE_STEPS: RaceStep[] = [
  ...
  { id: 4, label: 'First App Response', tcpRtt: 3, quicRtt: 2, tcpDesc: 'After handshake: request + response = 1 more RTT. Grand total: ~3 RTTs', quicDesc: 'After handshake: request + response = 1 more RTT. Grand total: ~2 RTTs' },
]
...
h3spec https://your-server.com:443`}</CodeBlock>
      ...
      <Para>Traditional load balancers route by TCP 5-tuple. QUIC connection migration changes the source IP/port, breaking 5-tuple routing. QUIC-aware load balancers must route by connection ID. RFC 9000 allows opaque server-chosen connection IDs; deployments can encode routing hints into IDs they generate so a load balancer can route without packet decryption. Cloudflare's implementation embeds a routing key into the connection ID for stateless L4 routing.</Para>
