...
export default function EmailProtocolsPage() {
  return (
    ...
    >
      ...
      <Para>The IDLE command keeps the IMAP connection open in a waiting state. When new mail arrives, the server sends an unsolicited EXISTS response — the client immediately receives the notification without polling. Many IMAP clients use it for push-like mail updates, though mobile platforms and providers may also use proprietary push services, Exchange ActiveSync, or app-specific APIs. The client sends <Code>IDLE\r\n</Code>, server responds <Code>+ idling</Code>, and both sides wait. Client terminates with <Code>DONE\r\n</Code>.</Para>
