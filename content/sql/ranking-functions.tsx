...
SELECT
  ...
  -- PERCENT_RANK (fractional rank 0–1, based on row position — not value)
  ROUND(PERCENT_RANK() OVER (
    ORDER BY lifetime_value
...
ORDER BY feb_rank;`}
      ...
      <IQ q="When would you use NTILE vs PERCENT_RANK for segmentation?">
        ...
        <p style={{ margin: '0 0 14px' }}>PERCENT_RANK is also position-based, not value-based — it is just as insensitive to the magnitude and spread of the underlying values as NTILE is. Both are purely a function of where a row sits in the sort order, not how big or small its actual value is. For a right-skewed distribution where the top 5% of customers account for 60% of revenue, PERCENT_RANK &gt;= 0.75 and NTILE(4) = 4 select essentially the same set of rows — the top 25% of customers by row count, not by value. Neither function "knows" that those top rows carry a disproportionate share of the total revenue.</p>
        <p style={{ margin: 0 }}>The real practical difference is tie handling. PERCENT_RANK gives every tied row the exact same fractional value — two rows with identical amounts always get identical PERCENT_RANK. NTILE has no such guarantee: when the bucket sizes don't divide evenly, NTILE force-splits rows into buckets by row order, so two rows with an identical value can land in different buckets if one falls on either side of a bucket boundary. Choose NTILE when you need a fixed number of equal-count segments and don't mind ties being split across them. Choose PERCENT_RANK when you need tied values to always land in the same category. When you actually need a magnitude-aware, value-based threshold — "the dollar amount where the top 25% of spend begins" — neither ranking function does that; reach for PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY col) instead, which interpolates an actual threshold value from the data.</p>
      </IQ>
      ...
      <Err
        ...
        cause="NTILE distributes rows as evenly as possible, but when total rows is not evenly divisible by n, some buckets get one extra row. NTILE(4) on 10 rows produces buckets of sizes 3, 3, 2, 2 — not all size 2 or 3. This is correct behaviour — NTILE makes no guarantee of exact equal sizes, only approximately equal sizes. A related surprise: because NTILE assigns buckets strictly by row order, two rows that share the identical ORDER BY value can end up split into different buckets if one lands on either side of a bucket boundary."
        fix="This is expected and correct — document it to stakeholders. The standard definition of NTILE distributes the remainder rows to the first buckets. If the goal is keeping tied values together, PERCENT_RANK solves that specific problem — WHERE PERCENT_RANK() OVER (ORDER BY col) >= 0.75 gives every tied row the identical fractional value, so ties never split across a boundary. Note that PERCENT_RANK does NOT solve unequal counts and is not value-based — it is just as row-position-based as NTILE, only continuous instead of bucketed. If a genuinely value-based, magnitude-aware threshold is required — not just position in the sort order — use PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY col) to compute the actual interpolated cutoff value and filter WHERE col >= that value. If exact row counts per bucket are required, calculate the target rows per bucket (FLOOR(COUNT(*) / n)) and use ROW_NUMBER with arithmetic to assign buckets manually."
      />
...
ORDER BY category, revenue_rank;`}
      ...
      <KeyTakeaways
        items={[
          ...
          'NTILE(n): equal-count buckets, purely row-position-based — just like PERCENT_RANK, it has zero awareness of value magnitude. When rows do not divide evenly by n, early buckets get one extra row, and ties can split across bucket boundaries. PERCENT_RANK differs mainly in tie handling (tied rows always share one value) and granularity (continuous 0.0–1.0, not fixed buckets). For a genuinely value-based, magnitude-aware threshold, use PERCENTILE_CONT instead.',
