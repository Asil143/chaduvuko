...
export { LearnLayout, KeyTakeaways }

export const S = '#29b5e8'
...
export const Divider = () => <div style={{ borderTop: '1px solid var(--border)', margin: '54px 0' }} />
...
export const Callout = ({ title, children, color = S }: { title: string; children: React.ReactNode; color?: string }) => (
...
export const CodeBox = ({ label, children }: { label: string; children: string }) => (
  ...
  <ul style={{ display: 'grid', gap: 11, margin: '0 0 22px', paddingLeft: 0, listStyle: 'none' }}>
    {items.map(item => (
      <li key={item} style={{ display: 'flex', gap: 11, alignItems: 'flex-start', fontSize: 14.5, color: 'var(--text)', lineHeight: 1.8 }}>
        <span style={{ color: S, fontWeight: 900, marginTop: 2 }}>✓</span>
        <span>{item}</span>
      </li>
    ))}
  </ul>
  ...
  <div style={{ overflowX: 'auto', marginBottom: 30 }}>
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5 }}>
      <thead>
        <tr>
          {headers.map(header => (
            <th key={header} style={{ padding: '11px 16px', textAlign: 'left', fontSize: 11, fontWeight: 800, letterSpacing: '.08em', textTransform: 'uppercase', color: S, fontFamily: FONT_MONO, borderBottom: `2px solid ${S}55`, background: `${S}0d`, minWidth: 180 }}>{header}</th>
          ))}
        </tr>
      </thead>
      <tbody>
