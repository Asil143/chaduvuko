...
interface Props {
  question: string;
  hint?: string;
  answer: string;         // The correct Python/pandas code (shown as answer)
...
export default function TryItChallenge({ question, hint, answer, explanation }: Props) {
  ...
  return (
    <div style={{
      border: `1px solid ${DS_COLOR}30`,
      borderRadius: 12,
      overflow: 'hidden',
      marginTop: 32,
      background: `${DS_COLOR}06`,
    }}>
      {/* Header */}
      <div style={{
        padding: '14px 20px',
        borderBottom: `1px solid ${DS_COLOR}20`,
        display: 'flex',
        alignItems: 'center',
        gap: 10,
        background: `${DS_COLOR}10`,
      }}>
        <span style={{
      ...
      <div style={{ padding: '14px 20px 16px', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        ...
        <button
          ...
          style={{
            background: answerOpen ? DS_COLOR : 'none',
            border: `1px solid ${DS_COLOR}`,
            color: answerOpen ? '#fff' : DS_COLOR,
            borderRadius: 6,
            padding: '6px 14px',
            fontSize: 12,
            cursor: 'pointer',
        ...
        <div style={{ margin: '0 20px 20px' }}>
          ...
          {explanation && (
            <div style={{
              marginTop: 12,
              padding: '12px 14px',
              background: `${DS_COLOR}08`,
              border: `1px solid ${DS_COLOR}20`,
              borderRadius: 8,
              fontSize: 13,
              color: 'var(--text)',
              lineHeight: 1.8,
            }}>
              <span style={{ color: DS_COLOR, fontWeight: 700, marginRight: 6 }}>Why this works:</span>
              {explanation}
            </div>
          )}
        </div>
