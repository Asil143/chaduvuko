...
export default function HtmlCssSectionNav({ slug }: { slug: string }) {
  const section = HTML_CSS_CURRICULUM.find(s => s.modules.some(m => m.slug === slug))
  ...
  return (
    <div style={{ marginBottom: 36 }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
        <Link href="/learn/html-css" style={{ fontSize: 12, color: 'var(--muted)', textDecoration: 'none', fontFamily: 'var(--font-mono)' }}>
          HTML &amp; CSS
        </Link>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 3, marginBottom: 8 }}>
          {section.modules.map(m => {
            ...
            const href = isLive ? `/learn/html-css/${m.slug}` : '#'
            return (
              <Link
                key={m.slug}
                href={href}
