...
const navItems = [
  { label: 'Roadmap', href: '/learn/roadmap' },
  {
    ...
    children: [
      ...
      { label: 'Data Engineering Roadmap', href: '/learn/roadmap' },
...
export function Navbar() {
  ...
  return (
    <nav
      ...
      style={{ background: 'rgba(var(--bg-raw,248,249,252),0.85)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--border)' }}
    >
      {/* Logo */}
      <Link href="/" className="flex items-center gap-1.5 font-display font-extrabold text-xl tracking-tight" style={{ color: 'var(--text)' }}>
        <span className="w-2 h-2 rounded-full" style={{ background: 'var(--accent)', animation: 'pulseDot 2s ease-in-out infinite' }} />
        ...
        {navItems.map(item => (
          ...
          >
            {item.href ? (
              ...
              <button className="flex items-center gap-1 px-3 py-2 rounded-lg text-sm transition-all" style={{ color: 'var(--muted)' }}>
      ...
      <div className="flex items-center gap-2">
        <ThemeToggle />
        <a href="/learn/roadmap" className="hidden md:flex btn-primary text-sm">
          Start Learning →
        </a>
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="absolute top-16 left-0 right-0 p-4 md:hidden z-40"
          style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)' }}>
          {navItems.map(item => (
            <div key={item.label}>
              {item.href ? (
                <Link href={item.href} className="block px-3 py-2.5 text-sm rounded-lg" style={{ color: 'var(--text2)' }}
                ...
                <>
                  ...
                  {item.children?.map(child => (
                    <Link key={child.href} href={child.href}
                      className="block px-5 py-2 text-sm rounded-lg" style={{ color: 'var(--text2)' }}
                      onClick={() => setMobileOpen(false)}>
                      → {child.label}
                    </Link>
                  ))}
